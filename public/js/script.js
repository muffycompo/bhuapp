 ...
 //    });
	...
	$('input#if_same').on('click', function(){
		...
		if($('input#if_same').is(':checked') === true){
			...
			// Empty fields value
			$('input#sponsor_name').attr('readonly', false).val('');
			$('textarea#sponsor_address').attr('readonly', false).val('');
			$('input#sponsor_gsm_no').attr('readonly', false).val('');
			$('input#sponsor_occupation').attr('readonly', false).val('');
		}
	});
